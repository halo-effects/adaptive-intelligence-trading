...
TRAILING_CALLBACK_PCT  = 0.2     # 0.2% trail distance after activation (was 0.5%, best performer in backtest)

# ── Liquidity Filter ────────────────────────────────────────────────────
# Coins must meet minimum 24h dollar volume to receive new allocations.
# The threshold scales with per-coin allocation to ensure orders are a
# small fraction of daily volume (target: <1% of 24h volume per order).
# MIN_VOLUME_MULTIPLIER=100 means: if a coin gets $40 allocation, it needs
# $4,000 in 24h volume.  This prevents slippage from eating the TP margin.
# MAX_ENTRY_SPREAD_BPS: if actual fill deviates more than this from engine
# price, the order is rejected and rolled back.
MIN_VOLUME_MULTIPLIER  = 100    # 24h $ volume must be >= allocation * this
...
class AsterPerpClient:
    ...
    def place_trailing_stop_sell(self, db_symbol: str, qty: float,
        ...
        base = db_symbol.split("/")[0]
        exchange_qty = qty * 1000 if base in ("PEPE", "BONK", "FLOKI") else qty
        # Round to exchange precision to avoid -1111 "Precision is over the maximum"
        try:
            exchange_qty = float(self._exchange.amount_to_precision(sym, exchange_qty))
            activation_price = float(self._exchange.price_to_precision(sym, activation_price))
        except Exception as e:
            logger.warning(f"Precision rounding failed for {db_symbol}: {e}")
        if self.dry_run:
...
class V14PortfolioLiveAster:
    ...
    def __init__(self, capital: float, confirm: bool = False,
        ...
        self._cumulative_realized_pnl: float = 0.0  # Accumulated realized PnL (for drift detection)
    ...
    def _load_state(self) -> bool:
        ...
        if "cumulative_realized_pnl" in state:
            self._cumulative_realized_pnl = state["cumulative_realized_pnl"]
            logger.info(f"Restored cumulative realized PnL: ${self._cumulative_realized_pnl:+.2f}")
    ...
    def _handle_tp_fill(self, sym: str, cs: CoinState, fill_result: dict):
        ...
        # Accumulate realized PnL for drift detection
        # This prevents realized profits from being misidentified as deposits.
        if record:
            self._cumulative_realized_pnl += record["pnl"]
            if abs(cs.cumulative_funding) > 0.001:
                self._cumulative_realized_pnl += cs.cumulative_funding
            logger.info(
                f"Cumulative realized PnL: ${self._cumulative_realized_pnl:+.2f} "
                f"(this deal: ${record['pnl']:+.2f})"
            )
    ...
    def _execute_action(self, sym: str, cs: CoinState, action: dict):
        ...
        if act_type == "BUY":
            ...
            # T1 entry gate: block first-layer entries for coins not in current allocations.
            # DCA add-on layers (L2+) on existing positions always pass — we never
            # strand an open position without capital to defend it.
            key = f"{sym}:long"
            is_t1 = key not in self.tracker._open_deals
            if is_t1 and sym not in self.router.active_allocations:
                logger.info(
                    f"T1 gate: blocking first entry for {sym} — "
                    f"not in current router allocations (pruned or never approved)"
                )
                if cs.engine:
                    cs.engine.reject_action(action)
                return
            ...
            if result and result.get("status") in ("filled", "dry_run"):
                ...
                # Audit #7: Spread logging + hard reject gate
                ...
                if spread_bps > MAX_ENTRY_SPREAD_BPS:
                    # Slippage too high — close the position immediately to limit damage.
                    # This is a market-sell of what we just bought, not a force-close of
                    # an existing DCA position.  The DCA grid never actually started.
                    logger.warning(
                        f"SPREAD REJECT: {sym} fill spread {spread_bps:.0f}bps > "
                        f"{MAX_ENTRY_SPREAD_BPS}bps limit. Closing immediately."
                    )
                    send_telegram(
                        f"⚠️ {TG_PREFIX} <b>Spread Reject</b>: {sym}\n"
                        f"Fill spread {spread_bps:.0f}bps exceeds {MAX_ENTRY_SPREAD_BPS}bps limit\n"
                        f"Engine: ${price:.6f} | Fill: ${actual_price:.6f}\n"
                        f"Closing position to avoid slippage loss."
                    )
                    try:
                    ...
                    self.router.return_capital(sym, granted)
                    return
                elif spread_bps > 50:  # > 0.5% warning
                    send_telegram(
            ...
            if result and result.get("status") in ("filled", "dry_run"):
                ...
                # Stale coin pruning after manual/forced close
                self._prune_stale_coin_after_tp(sym, cs)
    ...
    def _do_rebalance(self, current_dt: datetime):
        ...
        try:
            ...
            # ── Liquidity filter: remove coins with insufficient 24h volume ──
            ...
            try:
                ...
                for sym, alloc in allocations.items():
                    ...
                    if sym in self.coins:
                        cs = self.coins[sym]
                        if cs.engine and cs.engine._engine and (
                            cs.engine._engine.long_coins > 0 or cs.engine._engine.short_coins > 0
                        ):
                            filtered_allocs[sym] = alloc
                            continue
                    ...
                    # Threshold: max(allocation * multiplier, absolute floor)
                    min_vol = max(alloc * MIN_VOLUME_MULTIPLIER, MIN_VOLUME_FLOOR)

                    if quote_vol < min_vol:
                        logger.info(
                            f"Liquidity filter: {sym} excluded — "
                ...
                removed = set(allocations.keys()) - set(filtered_allocs.keys())
                if removed:
                    logger.info(f"Liquidity filter removed {len(removed)} coins: {sorted(removed)}")
    ...
    def _write_status(self):
        ...
        pnl_pct = ((equity - self._tracked_capital) / self._tracked_capital * 100) if self._tracked_capital > 0 else 0.0
        ...
        status = {
            ...
            "max_drawdown_pct": round(
                (self._tracked_capital - equity) / self._tracked_capital * 100
                if self._tracked_capital > 0 and equity < self._tracked_capital else 0.0, 2
            ),
        }
