#!/usr/bin/env python3
"""
V14-ETF Paper Trading Bot Runner
==================================
Entry point for V14 DCA-only lifecycle paper trading across 5 ETF-candidate
coins on Hyperliquid.

Coins: SOL, XRP, LTC, HBAR, ADA (all with pending/filed US spot ETFs)
...
Usage:
    python -m trading.spot.run_v14etf_paper [options]
    python trading/spot/run_v14etf_paper.py [options]
"""
...
CANONICAL_SYMBOLS = ["SOL/USDT", "XRP/USDT", "LTC/USDT", "HBAR/USDT", "ADA/USDT"]
...
# Canonical -> Hyperliquid price feed ticker (all perps — no spot for these)
HL_PRICE_MAP = {
...
DEFAULT_OUTPUT_DIR = _WORKSPACE / "trading" / "spot" / "paper" / "v14etf"
...
class V14PaperBot:
    ...
    def backfill(self):
        ...
        try:
            send_telegram(f"📊 <b>[V14-ETF]</b> starting backfill from {self.start_date}")
        except Exception:
            pass
        ...
        try:
            send_telegram(
                f"✅ <b>[V14-ETF] Backfill Complete</b>\n"
    ...
    def run_live(self):
        ...
        try:
            send_telegram("🔄 <b>[V14-ETF]</b> entering live trading mode")
        except Exception:
            pass
        ...
        while not self._shutdown:
            try:
                        ...
                        if actions:
                            ...
                            for act in actions:
                                ...
                                if a in ('SELL', 'SHORT_CLOSE'):
                                    ...
                                    try:
                                        send_telegram(
                                            f"{emoji} <b>[V14-ETF] {sym}</b> {a} @ ${act.get('price', 0):.4f}\n"
                                    ...
                                    try:
                                        send_telegram(
                                            f"🔄 <b>[V14-ETF] {sym}</b> Phase: {act.get('from')} → {act.get('to')}\n"
                ...
                try:
                    send_telegram(f"⚠️ <b>[V14-ETF] Error:</b> {str(e)[:200]}")
                except Exception:
                    pass
                time.sleep(30)
        ...
        try:
            send_telegram("🛑 <b>[V14-ETF]</b> stopped")
        except Exception:
            pass
    ...
    def _maybe_run_scanner(self):
        ...
        try:
            ...
            output_path = self.output_dir.parent.parent.parent / 'docs' / 'data' / 'v14etf' / 'scanner.json'
            ...
            try:
                send_telegram(
                    f"📊 <b>[V14-ETF] Scanner</b> daily refresh complete\n"
    ...
    def run(self, backfill_only=False, skip_backfill=False, fresh=False):
        ...
        if fresh:
            logger.info("Fresh start — no backfill, clean state, entering live immediately")
            try:
...
def main():
    parser = argparse.ArgumentParser(description="V14-ETF Paper Trading Bot")
    ...
    parser.add_argument("--fresh", action="store_true",
                        help="Fresh start — no backfill, clean state, enter live immediately")
