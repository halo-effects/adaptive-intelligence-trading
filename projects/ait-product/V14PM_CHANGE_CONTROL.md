...
| 2026-04-15 PM | **Trailing stop callback optimized: 0.5% → 0.2%** | `run_v14_portfolio_live_aster.py`: `TRAILING_CALLBACK_PCT` changed from 0.5 to 0.2 and default param `callback_rate` updated to match. **Rationale:** 365-day backtest on actual candle data (TAO, ZEC, FET, JTO, HYPE, $50K) showed 0.2% = $93,338 (186.7% ROI) vs 0.5% = $75,720 (151.4% ROI) vs fixed TP = $24,983 (50% ROI). **Exchange constraint:** Aster only accepts callback rates in 0.1% increments — 0.25% rejected with `-2007 Invalid callBack rate`. 0.2% is the best Aster-compatible option. Worst-case profit: 1.3% (activation 1.5% minus 0.2% callback). All 3 existing trailing stop orders cancelled and replaced with 0.2% callback. |
| 2026-04-15 PM | **Git source file protection** | `.gitignore`: Added all trading bot source code (`run_v14_*.py`, `v14_capital_manager.py`, `v14_dca_engine.py`, `v14_lifecycle_engine.py`) and runtime state files. Pushed to remote. `git rm --cached` on 3 tracked source files. **Root cause:** Two git processes (OpenClaw hourly backup cron + AIT_DashboardSync Windows task) both push to the same GitHub repo. When the sync does `git reset --soft origin/main` in its temp clone and pushes, then the backup cron does `git add -A` + push from the workspace, merges/rebases can silently drop local-only code changes. Fix ensures source files exist only on disk, never in git. |
| 2026-04-15 PM | **Shared source files restored from git history** | `v14_capital_manager.py`, `v14_dca_engine.py`, `v14_lifecycle_engine.py` restored from commit `f4526ff15` after git reset wiped them. These files are dependencies of all 4 bots. Paper bots continue on fixed TP (Phase 2 trailing code deferred). |
| 2026-04-15 PM | **Tier cap enforcement re-verified** | `run_v14_portfolio_live_aster.py`: Confirmed `active_count` guard still present after code restore. Prevents rebalance from creating new engines beyond tier coin cap when existing positions fall off scanner top-N. |
