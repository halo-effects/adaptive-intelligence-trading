...
| 2026-04-13 PM | **30% order cap removed** | `v14_dca_engine.py`: Removed paper-only 30% capital cap that inverted Martingale multiplier sizing. CapitalRouter manages allocation limits for both live and paper. `live_mode` flag re-enabled for live bots. |
| 2026-04-14 AM | **INCIDENT: Accidental code revert** | Auto-backup committed reverted code. PM live runner deleted (3,120 lines), trailing stop stripped from DCA engine, capital manager regressed. Bot threw `NameError: name 'low' is not defined` on every tick from ~10 PM. See §17.8 in architecture doc. |
| 2026-04-15 AM | **Code restoration + bug fix** | All files restored from commit `1554a0500`. Fixed: `_long_dca_tick` missing `low` parameter, lifecycle engine not passing `high` to `_short_dca_tick`. PM live bot restarted with 7 coins. |
| 2026-04-15 AM | **SAFEGUARD: .gitignore code/data separation** | Bot runtime data (`status.json`, `state.json`, `trades.csv`, logs, locks, PIDs) removed from git tracking. Wildcard patterns auto-cover new bot instances. Auto-backup cron updated. Prevents future accidental code regression from data file churn. |
| 2026-04-15 AM | **Tier cap enforcement bug fix** | `run_v14_portfolio_live_aster.py`: Fixed rebalance overshoot — new engine creation now gated by counting active positions (`long_coins > 0 or short_coins > 0`) against `tier_coin_cap`. Previously, cap only limited scanner selection but not total engine count, so manually closing a position mid-cycle exposed the bug: rebalance added N new coins without checking existing open positions. Added `active_count` guard, logged as "Rebalance gate: X active positions, tier cap = Y". Increments count per new engine added. |
| 2026-04-15 AM | **Source file protection** | `run_v14_portfolio_live_aster.py` and `v14_capital_manager.py` added to `.gitignore` and untracked from git. Data sync merges with `origin/main` were deleting these files because remote doesn't track them. Files restored from commit `f4526ff15`. |
| 2026-04-15 AM | **State edit procedure documented** | Architecture doc §6.2 updated: Never edit `state.json` while bot is running (bot overwrites every ~60s). Correct sequence: kill bot → edit state → make exchange changes → restart. Incident: pruned state from 7 to 3 engines while bot was live, bot's next state save overwrote the prune. |
| 2026-04-15 AM | **Position prune to 3 coins** | Closed 4 excess positions (GRASS, FET, ZEC, RENDER) via market sell. Pruned state to TAO + HYPE + JTO (3-coin tier at $374 equity). All 3 TP orders confirmed as TRAILING_STOP_MARKET on exchange. |
