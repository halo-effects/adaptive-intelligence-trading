...
_Created: 2026-04-13 | Status: APPROVED FOR IMPLEMENTATION_
...
After activation, the stop trails price upward with a 0.5% callback distance.

**Zero-downside design:** Trail only activates after the current TP level is reached.
Worst case = same as today. Best case = captures runaway moves (+95% extra profit in backtest).
...
- [ ] place_trailing_stop_sell() works with PEPE/BONK/FLOKI qty scaling
- [ ] _check_tp_fills() correctly detects trailing stop fill
- [ ] _handle_tp_fill() records correct fill price and PnL
- [ ] _recover_tp_orders() recognizes trailing stop orders on restart
- [ ] Feature flag OFF falls back to limit sell cleanly
- [ ] Trailing stop fallback to limit sell when exchange rejects
- [ ] State persistence: tp_type, tp_activation_price survive restart
...
### Phase 3 (Dashboard)
- [ ] Trail badge displays on positions with trailing TP
- [ ] Status badge shows trailing enabled
