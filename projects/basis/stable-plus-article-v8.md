...
Under the hood, all Stable+ and Floor+ tokens on Basis are paired with **Stasis** — the platform's native Stable+ token, which is itself paired with USDT. Stasis uses the same Stable+ mechanics, so its value in USDT only increases over time.
...
Stable+ and Floor+ aren't standard ERC-20 tokens dropped onto an existing exchange. The token mechanics are deeply integrated with the Basis DEX — the exchange architecture was engineered from the ground up specifically to make this token technology work safely and correctly.

The elastic supply model, the embedded reserve system, the pricing engine, MEV protection, and the one-way ratchet are all tightly coupled with the exchange itself. These aren't features bolted onto a generic DEX — they're inseparable from it. The token contracts, the trading engine, and the reserve management all work as a single integrated system.

**This is what makes Basis uncopyable.** You can't take this token technology and deploy it on PancakeSwap, Uniswap, or any other existing DEX. Their routers, liquidity models, and fee structures were designed for fixed-supply tokens with external LP pools — a fundamentally different architecture. Attempting to replicate Stable+ or Floor+ mechanics on infrastructure that wasn't built for them would break the safety guarantees that make the tokens work.

For users, this means one thing: **Basis tokens launched on the Basis DEX are guaranteed to behave correctly.** The exchange enforces the rules. No one can deploy a broken or malicious imitation of the token mechanics because the mechanics only function within the Basis architecture.

Stable+ and Floor+ tokens can also be listed on other DEXs with a separate liquidity pool for broader access, but the Basis DEX is always the source of truth for price discovery. Other DEX listings are secondary markets — arbitraged against the Basis price.
...
**No separate LP pool.** The real liquidity (USDT) lives inside the token contract itself. There are no LP tokens to drain. No external pool to rug. The liquidity IS the contract.

### The Simple Version

Every Basis token starts at $1, backed 1:1 by real USDT in the contract. When someone buys, new tokens are minted. When someone sells, tokens are burned.

Here's what makes Stable+ different: the pricing engine is designed so that every trade — buy or sell — leaves a small surplus in the reserve. You always get slightly less than a pure 1:1 exchange would give you, and that difference stays in the pool.

Over time, this means more USDT backing fewer tokens. The price per token ratchets up. Permanently.

The price is a property of two things working together: the full liquidity reserve and the proprietary pricing mechanics built into the Basis DEX. The price can only increase.
...
Here's why that doesn't apply: every buy adds real USDT to the pool. There are no phantom assets. No leverage. No borrowed reserves. The pool holds exactly the USDT that was put in (minus what was withdrawn through sells, plus the surplus retained by the pricing engine).
