...
All Floor+ tokens on Basis are paired with **Stasis** — the platform's native Stable+ token. Stasis itself is paired with USDT and uses Stable+ mechanics, so its value in USDT only increases over time.

This means Floor+ tokens benefit from the same double appreciation as Stable+ tokens: the floor rises in Stasis terms, and Stasis rises in USDT terms. In dollar value, your floor is rising on two axes.
...
**No separate LP pool.** The real Stasis liquidity lives inside the token contract. No external pool to drain, no LP tokens to rug.

The constant product formula is what creates the surplus. On every buy, you receive slightly fewer tokens than your Stasis would suggest at a flat rate. On every sell, you receive slightly less Stasis back than the spot price would suggest. That "slightly less" — in both directions — stays in the reserve.

After a complete cycle of buying and selling, even if every token is sold and burned, there is more Stasis in the reserve than at launch. The floor price is permanently higher.
