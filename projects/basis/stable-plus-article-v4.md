...
*A token with a mathematically enforced price that only rises. Stable+ tokens on Basis are paired with a stablecoin — for example USDT, which we'll use throughout this article.*
...
Stable+ is paired with USDT, but the mechanics are fundamentally different. Stable+ isn't trying to maintain a peg at all — the price in USDT simply increases over time. There is no target price to defend. The constant product formula and the 1:1 reserve guarantee that the price can only go in one direction: up. There is nothing to "break."
