...
Under the hood, all Floor+ tokens on Basis are paired with **Stasis** — the platform's native Stable+ token, which is itself paired with USDT. Stasis uses Stable+ mechanics, so its value in USDT only increases over time.
...
## Built on a Purpose-Built DEX

Floor+ tokens aren't standard ERC-20 tokens dropped onto an existing exchange. The token mechanics are deeply integrated with the Basis DEX — the exchange architecture was engineered from the ground up specifically to make this token technology work safely and correctly.

The elastic supply model, the embedded reserve system, the pricing engine, MEV protection, and the stability dial are all tightly coupled with the exchange itself. These aren't features bolted onto a generic DEX — they're inseparable from it. The token contracts, the trading engine, and the reserve management all work as a single integrated system.

**This is what makes Basis uncopyable.** You can't take this token technology and deploy it on PancakeSwap, Uniswap, or any other existing DEX. Their routers, liquidity models, and fee structures were designed for fixed-supply tokens with external LP pools — a fundamentally different architecture. Attempting to replicate Floor+ mechanics on infrastructure that wasn't built for them would break the safety guarantees that make the tokens work.

For users, this means one thing: **Floor+ tokens launched on the Basis DEX are guaranteed to behave correctly.** The exchange enforces the rules. No one can deploy a broken or malicious imitation of the token mechanics because the mechanics only function within the Basis architecture.

Floor+ tokens can also be listed on other DEXs with a separate liquidity pool for broader access, but the Basis DEX is always the source of truth for price discovery. Other DEX listings are secondary markets — arbitraged against the Basis price.
...
The floor price is backed by real USDT reserves held inside the token contract itself — not in a separate LP pool. There are no LP tokens to drain. The liquidity IS the contract. Every transaction leaves a small surplus in the reserve, and that surplus pushes the floor higher. The floor can never decrease. It's a one-way ratchet, enforced by the contract and the Basis DEX working together.
...
When a Floor+ token is created, the creator sets a stability dial — a value between 50% and 90%. This controls how fast the price moves and how much volatility the token experiences.
...
**Low stability (50%)** means the price swings harder. More room for the price to run — and more room for it to fall before hitting the floor. Think of it as a growth stock with a guaranteed buyback price.
...
**No separate LP pool.** The real USDT liquidity lives inside the token contract. No external pool to drain, no LP tokens to rug.

The Basis pricing engine is designed so that every trade — buy or sell — leaves a small surplus in the reserve. After a complete cycle of buying and selling, even if every token is sold and burned, there is more USDT in the reserve than at launch. The floor price is permanently higher.

The stability dial controls the speed: lower stability = bigger price swings, faster floor accumulation per trade. Higher stability = smaller swings, steadier accumulation. But the floor rises at every setting.
...
## The Phantom Value Problem (And Why Floor+ Solves It)
...
**Floor+ eliminates phantom value.** The reserves backing each token are real and embedded in the contract — not sitting in a separate, drainable LP pool. When the floor says $X, there is actually $X worth of USDT per token held in the contract. The floor price isn't a theoretical calculation — it's a guaranteed exit price.

Above the floor, spot price discovery still happens normally. But even at the floor, you know exactly what you can get — because the reserves are there, enforced by the contract.

For any serious holder — agents managing capital, treasuries parking funds, traders sizing positions — this is the difference between real value and a number on a screen.
...
- When anyone sells, tokens are burned and the surplus stays in the reserve. The floor rises.
...
For agents, traders, and creators building portfolios, launching communities, or managing treasuries — this is the risk profile that finally makes sense. Not "we hope it doesn't crash." Not "trust us." Just math and real reserves — and a floor that only goes up.
