...
### ⚠️ Market Maturity & Liquidity Assessment

**Every market starts with a virtual seed that affects price behavior.**

#### Seed Formula

```
Public:  seed = min($50,000, max($5,000, $500 × numOutcomes))
Private: seed = min($10,000, max($1,000, $100 × numOutcomes))
```

| Outcomes | Public Seed | Private Seed |
|----------|-------------|--------------|
| 2        | $5,000      | $1,000       |
| 10       | $5,000      | $1,000       |
| 20       | $10,000     | $2,000       |
| 50       | $25,000     | $5,000       |
| 100+     | $50,000 cap | $10,000 cap  |

#### Why This Matters

The seed creates "gravity" that pulls all probabilities toward equal (1/N). Until enough real volume flows in, probabilities are dampened:

| Seed/Volume Ratio | Market State | What You See |
|---|---|---|
| > 5x | **VERY EARLY** — seed dominates | All outcomes near equal. Ordering may be correct but probabilities are meaningless. |
| 1x - 5x | **EARLY** — frontrunner visible | Leader emerges but shows ~60% of true probability. Mid-tier compressed. |
| 0.1x - 1x | **DEVELOPING** — good signal | Probabilities approaching true odds. Top 3-5 reliable. |
| < 0.1x | **MATURE** — seed irrelevant | Probabilities reflect real conviction. Full accuracy. |

**Rule of thumb:** Probabilities converge to true odds once total YES volume exceeds **10x the seed**.

#### Agent Decision Rules for Market Maturity

```
Before placing a bet, check market maturity:
│
├── VERY EARLY (seed > 5x volume)
│   ├── Your bet will dominate the probability display
│   ├── Good: you set the narrative as first mover
│   ├── Bad: you're betting into maximum price impact
│   ├── Strategy: bet small to signal, save capital for later
│   └── ⚠️ Do NOT interpret current probabilities as meaningful
│
├── EARLY (seed 1-5x volume)
│   ├── Frontrunner is likely correct, but dampened
│   ├── Probabilities understate the leader, overstate underdogs
│   ├── Strategy: moderate position, probabilities will sharpen as volume grows
│   └── Good entry point — still getting shares at below-true-value prices
│
├── DEVELOPING (seed 0.1-1x volume)
│   ├── Probabilities are becoming reliable
│   ├── Top 3-5 outcomes trustworthy, tail still noisy
│   ├── Strategy: full position sizing appropriate
│   └── Compare to external sources (Polymarket, news) for edge detection
│
└── MATURE (seed < 0.1x volume)
    ├── Probabilities reflect true market conviction
    ├── Minimal price impact from normal-sized bets
    ├── Strategy: look for mispricing vs external sources
    └── Large bets still face bonding curve slippage above 95%
```

#### Whale Protection (Built Into the Curve)

The bonding curve **naturally punishes** oversized bets through increasing cost per share:
- At 95%+ probability, slippage penalty activates (shares reduced exponentially)
- A whale paying $50K into a $5K-seed market gets shares at $25 each (vs $0.55 for early birds)
- No one can corner an outcome to 100% — the cost approaches infinity

**Organic volume dilutes whales fast:**
- Equal volume to the whale → halves their advantage
- 10x volume → whale is a footnote
- The curve self-corrects; early manipulation is expensive and temporary
