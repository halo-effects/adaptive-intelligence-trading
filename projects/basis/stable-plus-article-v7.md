...
Stable+ and Floor+ aren't standard ERC-20 tokens dropped onto an existing exchange. The Basis DEX was engineered from the ground up specifically for this token technology — elastic supply, embedded reserves, the constant product formula, MEV protection, and the one-way ratchet are all native to the exchange architecture.
...
Stable+ and Floor+ tokens can be listed on other DEXs with a separate liquidity pool, but the Basis DEX is always the source of truth for price discovery. The token contract, the reserves, and the constant product formula all live on Basis. Other DEX listings are secondary markets — arbitraged against the Basis price.
...
Stable+ is fundamentally different. It isn't trying to maintain a peg at all — the price simply increases over time. There is no target price to defend. The constant product formula and the 1:1 reserve guarantee that the price can only go in one direction: up. There is nothing to "break."
