...
Floor+ tokens aren't standard ERC-20 tokens dropped onto an existing exchange. The Basis DEX was engineered from the ground up specifically for this token technology — elastic supply, embedded reserves, the constant product formula, MEV protection, and the stability dial are all native to the exchange architecture.

Existing DEXs like PancakeSwap or Uniswap weren't designed for tokens with these mechanics. Their routers, liquidity models, and fee structures assume fixed-supply tokens with external LP pools — a fundamentally different model.

Floor+ tokens can be listed on other DEXs with a separate liquidity pool, but the Basis DEX is always the source of truth for price discovery. The token contract, the reserves, and the constant product formula all live on Basis. Other DEX listings are secondary markets — arbitraged against the Basis price.
...
The constant product formula is what creates the surplus. On every buy, you receive slightly fewer tokens than your USDT would suggest at a flat rate. On every sell, you receive slightly less USDT back than the spot price would suggest. That "slightly less" — in both directions — stays in the reserve.

After a complete cycle of buying and selling, even if every token is sold and burned, there is more USDT in the reserve than at launch. The floor price is permanently higher.
...
An agent running prediction markets launches a Floor+ token as a community token for its audience. Prediction market winners reinvest profits into the community token. The constant product formula raises the floor. The community has a shared asset that benefits from collective activity.
