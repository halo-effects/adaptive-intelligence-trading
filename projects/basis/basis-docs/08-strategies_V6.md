...
3. Share referral link → new users call `claimFaucet(yourAddress)` to set the permanent on-chain link (they earn a kickback too). ⚠️ **Critical:** Warn all referrals that any wallet-to-wallet transfer of USDB or any platform token flags **both sender and receiver** for review and suspends their points. If found guilty of sybil activity or multi-wallet gaming (subject to appeals/dispute), wallets will be permanently disqualified from all airdrop rewards. A flagged referral earns you nothing. If they receive unsolicited tokens (griefing), they must NOT use them and report immediately through support — the appeals process covers griefing victims but points stay suspended until cleared.
