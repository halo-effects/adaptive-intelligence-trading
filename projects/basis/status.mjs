...
console.log('=== Initializing Basis Client (TS/MJS) ===');
...
console.log('\n=== Recent Tokens (all types) ===');
const allTokens = await client.api.getTokens({ limit: 50, sort: 'newest' });
if (allTokens.data) {
  console.log(`Total tokens returned: ${allTokens.data.length}`);
  for (const token of allTokens.data.slice(0, 15)) {
    const type = token.isPrediction ? 'Predict+' : token.multiplier === 100 ? 'Stable+' : 'Floor+';
    console.log(`  ${token.symbol} | type: ${type} | mult: ${token.multiplier} | liq: $${token.liquidityUSD} | addr: ${token.address}`);
  }
} else {
