...
// Pass the old (potentially invalid) API key to satisfy init
const client = await BasisClient.create({ privateKey: env.BASIS_PRIVATE_KEY, apiKey: env.BASIS_API_KEY });

// Delete the old key
console.log('Deleting old API key: cmnuud7v500061fl1t9t2xkmj');
try {
...
try {
  const newKey = await client.api.createApiKey('geegee-v2');
