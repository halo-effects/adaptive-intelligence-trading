...
const buyAmounts = [10, 50, 100, 500, 1000];

async function simulate(token, usdAmount) {
  const rawAmount = parseUnits(String(usdAmount), 18);
  try {
    const result = await client.readContract({
      address: SWAP,
      abi: SWAP_ABI,
      functionName: 'getAmountsOut',
      args: [rawAmount, [USDB, STASIS, token.address]],
    });
    const tokensOut = parseFloat(formatUnits(result, 18));
    const effectivePrice = usdAmount / tokensOut;
    return { tokensOut: tokensOut.toFixed(2), effectivePrice: effectivePrice.toFixed(4) };
  } catch (e) {
    return { tokensOut: 'ERROR', effectivePrice: 'ERROR' };
  }
}

async function main() {
  console.log('hybridMultiplier Comparison Table (all startLP=1000)');
  console.log('====================================================\n');
  
  // Header
  const header = 'Hybrid | ' + buyAmounts.map(a => `$${a} buy`).join(' | ');
  console.log(header);
  ...
  for (const token of tokens) {
    ...
    for (const amt of buyAmounts) {
      const sim = await simulate(token, amt);
      results.push(`$${sim.effectivePrice}`);
    }
    console.log(`  ${String(token.hybrid).padStart(3)} | ${results.join(' | ')}`);
  }
  
  console.log('\n\nDetailed (tokens received per buy amount):');
  console.log('===========================================\n');
  
  const header2 = 'Hybrid | ' + buyAmounts.map(a => `$${a}`).join(' | ');
  console.log(header2);
