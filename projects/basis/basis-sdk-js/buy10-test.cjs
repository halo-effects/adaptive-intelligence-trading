...
async function main() {
  console.log('Prices BEFORE $10 buy:');
  ...
  console.log('\nBuying $10 on each...\n');
  const BUY = parseUnits('10', 18);

  for (const t of tokens) {
    try {
      const { request } = await publicClient.simulateContract({
        account, address: SWAP, abi: SWAP_ABI, functionName: 'buyTokens',
        args: [BUY, 0n, [USDB, STASIS, t.a], false],
      });
      const hash = await walletClient.writeContract(request);
      await publicClient.waitForTransactionReceipt({ hash });
      console.log(`  ${t.s}: OK`);
    } catch (e) {
      console.log(`  ${t.s}: ERROR - ${(e.shortMessage || e.message).slice(0, 100)}`);
    }
    await new Promise(r => setTimeout(r, 2000));
  }

  console.log('\nPrices AFTER $10 buy (cumulative $11 total):');
  for (const t of tokens) {
    const p = await getPrice(t.a);
    console.log(`  ${t.s} (h=${t.h}): $${p.toFixed(6)}`);
  }
