...
Not because of a peg. Not because a team promises to buy back. But because the smart contract's math — backed by real reserves — makes a price decrease physically impossible.

That's Stable+. And it's the strangest thing in DeFi right now.

## Stasis: The Base Layer

All Stable+ and Floor+ tokens on Basis are paired with **Stasis** — the platform's native Stable+ token. Stasis itself is paired with USDT and uses the same Stable+ mechanics, so its value in USDT only increases over time.

This creates a double appreciation effect: your Stable+ token's price rises in Stasis, and Stasis's price rises in USDT. In dollar terms, you benefit from both.
...
**No separate LP pool.** The real liquidity (Stasis) lives inside the token contract itself. There are no LP tokens to drain. No external pool to rug. The liquidity IS the contract.

### The Simple Version

Start with the basics. Imagine a token with a 100% liquidity reserve — no fancy math:

- Reserve: 1,000 Stasis · Tokens: 1,000 · Price: 1.00 Stasis
- Someone buys 1 Stasis worth → Reserve: 1,001 · Tokens: 1,001 · Price: 1.00 Stasis

Every token is backed 1:1. The price never moves. Simple. All Basis tokens start at 1 Stasis.

Now here's what Stable+ does differently: it uses a constant product formula, so your 1 Stasis buy mints *slightly less* than 1 token. The extra stays in the pool.

- Reserve: 1,001 Stasis · Tokens: 1,000.99 · Price: 1.00001 Stasis

Sells work the same way. Burning 1 token at a 1 Stasis price gives you 0.99 Stasis back — the difference stays in the liquidity reserve.

Every trade — buy or sell — leaves slightly more Stasis backing each remaining token. Multiply across thousands of trades and the price ratchets up permanently.
...
If 90% of holders sell, each sell leaves a tiny surplus in the reserve. The remaining 10% of holders now have their tokens backed by more Stasis per token. The price has risen.
...
Stable+ is not pegged to anything. Every Basis token starts at 1 Stasis, but the price only goes up from there. It's not stable in the "stays flat" sense — it's stable in the "never goes down" sense. Different concept entirely.
...
Stable+ is paired with Stasis, but the mechanics are fundamentally different. Stable+ isn't trying to maintain a peg at all — the price in Stasis simply increases over time. There is no target price to defend. The constant product formula and the 1:1 reserve guarantee that the price can only go in one direction: up. There is nothing to "break."

### It's not a Ponzi or pyramid.

The common skeptic response: "If the price can only go up, someone has to lose."

Here's why that doesn't apply: every buy adds real Stasis to the pool. There are no phantom assets. No leverage. No borrowed reserves. The pool holds exactly the Stasis that was put in (minus what was withdrawn through sells, plus the surplus retained by the constant product formula).

When a seller exits, they get real Stasis from the pool, and the remaining holders have a higher price. The "last person in" isn't left holding depreciating tokens — they're holding a token at the highest price in the token's history.
...
Your Stable+ tokens are locked as collateral. You borrow against their full value. The price continues to rise while your tokens are locked (from other people's trading activity). When you repay the loan, your tokens are worth more than when you deposited them.
...
On Floor+ tokens, the spot price can trade above the floor, meaning leverage is calculated against the lower floor value. On Stable+, every unit of collateral counts at full value. This is why Stable+ supports the highest leverage ratios on the platform.
...
But here's what makes it powerful: the outcome token gives you ways to profit beyond just betting. You can buy the token for price appreciation. You can use it as loan collateral. You can combine strategies — buy the token, take a loan against it, and use the loan proceeds to place bets. The token and the prediction market are separate instruments that work together.
...
**Savings Instruments:** Users converting value into Stable+ tokens as a savings strategy. Not yield farming with its risks — just a token that mathematically can't lose value.
