...
def create_modified_engine():
    ...
    # Add CFGI hard gate logic
    # Insert after the price discovery veto logic
    veto_insertion_point = 'logger.info("  🚫 PRICE DISCOVERY VETO: EXIT vetoed — weekly doesn\'t confirm bearish structure")'
    
    if veto_insertion_point in engine_code:
        # Find the right place to insert CFGI gate logic
        # We need to add CFGI checks in multiple locations where exits are triggered
        
        # Add CFGI gate method to DailyScorerConductor class
        cfgi_gate_method = '''
...
'''
        ...
        # Add CFGI gate checks in the main exit logic
        # Find and modify the first exit trigger
        exit_logic_1 = '''                if self._conductor._in_price_discovery:
                    if self._conductor.weekly_confirms_exit_price_discovery(
                            getattr(self, '_accumulated_1h', None), price):
                        self._transition_to_exit(price, ts, ts_ms, daily_score)
                    else:
                        logger.info("  🚫 PRICE DISCOVERY VETO: EXIT vetoed — weekly doesn't confirm bearish structure")'''
        
        exit_logic_1_improved = '''                if self._conductor._in_price_discovery:
                    if self._conductor.weekly_confirms_exit_price_discovery(
                            getattr(self, '_accumulated_1h', None), price):
                        if self._conductor.cfgi_allows_exit(ts_ms):
                            self._transition_to_exit(price, ts, ts_ms, daily_score)
                        # CFGI veto is logged in cfgi_allows_exit
                    else:
                        logger.info("  🚫 PRICE DISCOVERY VETO: EXIT vetoed — weekly doesn't confirm bearish structure")'''
        
        engine_code = engine_code.replace(exit_logic_1, exit_logic_1_improved)
        
        # Add CFGI gate to weekly distribution veto path
        exit_logic_2 = '''                elif self._v12_weekly_dist_veto:
        ...
        exit_logic_2_improved = '''                elif self._v12_weekly_dist_veto:
                    if self._conductor.weekly_confirms_exit(
                            getattr(self, '_accumulated_1h', None), "distribution"):
                        if self._conductor.cfgi_allows_exit(ts_ms):
                            self._transition_to_exit(price, ts, ts_ms, daily_score)
        ...
        engine_code = engine_code.replace(exit_logic_2, exit_logic_2_improved)
        
        # Add CFGI gate to direct exit path
        exit_logic_3 = '''                else:
                    self._transition_to_exit(price, ts, ts_ms, daily_score)'''
        
        exit_logic_3_improved = '''                else:
                    if self._conductor.cfgi_allows_exit(ts_ms):
                        self._transition_to_exit(price, ts, ts_ms, daily_score)
                    # CFGI veto is logged in cfgi_allows_exit'''
        
        engine_code = engine_code.replace(exit_logic_3, exit_logic_3_improved)
        
        # Find and modify the second exit trigger (in the else block for MARKUP phase)
        markup_exit_logic = '''            if self._conductor.should_exit(ts_ms, price, fg_value):
                # Price discovery mode: price > KNOWN_ATH, ATH gate is useless.
                # Require robust weekly bearish structure confirmation instead.
        ...
        markup_exit_logic_improved = '''            if self._conductor.should_exit(ts_ms, price, fg_value):
                # Price discovery mode: price > KNOWN_ATH, ATH gate is useless.
                # Require robust weekly bearish structure confirmation instead.
                if self._conductor._in_price_discovery:
                    if self._conductor.weekly_confirms_exit_price_discovery(
        ...
        if markup_exit_logic in engine_code:
            engine_code = engine_code.replace(markup_exit_logic, markup_exit_logic_improved)
        
        # Handle the MARKUP->EXIT transition (third location)
        markup_weekly_veto = '''                elif self._v12_weekly_dist_veto:
                    if self._conductor.weekly_confirms_exit(
                            getattr(self, '_accumulated_1h', None), "distribution"):
                        self._transition_to_exit(price, ts, ts_ms, daily_score)
                    else:
        ...
        markup_weekly_veto_improved = '''                elif self._v12_weekly_dist_veto:
                    if self._conductor.weekly_confirms_exit(
                            getattr(self, '_accumulated_1h', None), "distribution"):
                        if self._conductor.cfgi_allows_exit(ts_ms):
                            self._transition_to_exit(price, ts, ts_ms, daily_score)
        ...
        if markup_weekly_veto in engine_code:
            engine_code = engine_code.replace(markup_weekly_veto, markup_weekly_veto_improved)
        
        # Handle MARKUP direct exit path
        markup_direct_exit = '''                else:
                    self._transition_to_exit(price, ts, ts_ms, daily_score)'''
        
        # Find the MARKUP->EXIT direct path by looking for a more specific pattern
        lines = engine_code.split('\n')
        for i, line in enumerate(lines):
            if 'self._transition_to_exit(price, ts, ts_ms, daily_score)' in line:
                # Check if this is in a simple else block
                if i > 0 and lines[i-1].strip() == 'else:':
                    # This is likely a direct exit - add CFGI gate
