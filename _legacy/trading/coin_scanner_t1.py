    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                ...
                print(f"  [rate-limited] {aster_symbol}, waiting {wait}s (attempt {attempt+1}/3)")
                time.sleep(wait)
                continue
            r.raise_for_status()
            data = r.json()
            ...
            if attempt < 2:
                ...
                continue
            print(f"  [error] {aster_symbol} after 3 attempts: {e}")
            return None
    return None
...
# ── CFGI-eligible tokens (only scan what we can trade with proper CFGI) ──
# Tokens with coin-specific CFGI that are tradeable on Aster or Hyperliquid
CFGI_ELIGIBLE_PAIRS = {
    # Aster USDT pairs
    ...
    "SOLUSDT": "SOL/USDT",
    "XRPUSDT": "XRP/USDT",
    # Note: HYPE, PEPE, TRUMP are Hyperliquid-only (USDC pairs)
    # They're handled separately in run_scanner.py if needed
}


def run_tier1(top_n: int = 15, cfgi_only: bool = True) -> list[dict]:
    """Run Tier 1 scan. By default only scans CFGI-eligible coins.
    
    Set cfgi_only=False for legacy full-exchange scan (291 pairs).
    """
    print("=" * 70)
    ...
    if cfgi_only:
        # Only scan coins with coin-specific CFGI
        print(f"\n[1] CFGI-only mode: scanning {len(CFGI_ELIGIBLE_PAIRS)} eligible pairs")
        aster_pairs = list(CFGI_ELIGIBLE_PAIRS.keys())
        print(f"    Coins: {', '.join(CFGI_ELIGIBLE_PAIRS.values())}")
        # Skip maturity filters — these are pre-vetted established coins
        mature_pairs = aster_pairs
        rejected = []
        filter_counts = {}
    else:
        # Legacy: scan all Aster pairs with maturity filters
    ...
    # ── Technical scoring ──
    step = "[2]" if cfgi_only else "[3]"
    print(f"\n{step} Fetching 1h data & scoring ({len(mature_pairs)} pairs)...")
    results = []
