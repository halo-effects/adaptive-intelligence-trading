...
// CONFIG
var CONFIG = {
  statusUrl: 'data/v14/status.json',
  tradesUrl: 'data/v14/trades.csv',
...
var COIN_BASE = {
  ...
  ATOM:{icon:'atom',label:'ATOM',name:'Cosmos',color:'#6F7390'},
  LINK:{icon:'link',label:'LINK',name:'Chainlink',color:'#2A5ADA'},
  NEAR:{icon:'near',label:'NEAR',name:'NEAR',color:'#00C08B'}
};
function cm(sym){var b=sym.split('/')[0];return COIN_BASE[b]||{icon:'hbar',label:b,name:b,color:'#888'}}
...
function renderFlowDiagram(){
  ...
  if(tpEl&&scannerData&&scannerData.top_picks){
    ...
    // Find scores for top picks from current window
  ...
  syms.forEach(function(sym){
    ...
    // Trend score from scanner history
    var trendData=null;
  ...
  // Sort by dca_score descending (nulls last)
